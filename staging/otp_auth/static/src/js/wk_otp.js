...
odoo.define('otp_auth.wk_otp', function (require) {
    ...
    $(document).ready(function() {
        var ValidUser = 0;
        if ($('#otpcounter').get(0)) {
            $("#otpcounter").html("<a class='btn btn-link pull-left wk_send' href='#'>" + _t("أرسل الكود") + "</a>");
        ...
        $('.wk_send').on('click', function(e) {
            ...
            if (email) {
                if(validateEmail(email)) {
                    ...
                    $('#wk_error').remove();
                    $(".field-confirm_password").after("<p id='wk_error' class='alert alert-danger'>! من فضلك تأكد من إدخال بريد الإكتروني صحيح.</p>");
                }
            } else {
                $('#wk_error').remove();
                $(".field-confirm_password").after("<p id='wk_error' class='alert alert-danger'>الرجاء إدخال عنوان البريد الإلكتروني.</p>");
            }
        });
    ...
    function getInterval(otpTimeLimit) {
        ...
        var x = setInterval(function() {
            ...
            if (countDown < 0) {
                clearInterval(x);
                $("#otpcounter").html("<a class='btn btn-link pull-right wk_resend position-relative' href='#'>إعادة أرسال الكود</a>");
                $("#otpcounter").append('<span>أنتهت صلاحية الكود. من فضلك قم بالضغط على زر إعادة أرسال الكود</span>');
                $(":submit").attr("disabled", true);
            }
        }, 1000);
    }
