...
class PatientTimeline(models.Model):
    ...
    * Conceptually we can use the widget on anyfield. 
    ...
    def _compute_family_history(self):
        ...
        if family_history:
            ...
            for field_name, (field_label, custom_field_name) in fields_to_process.items():
                ...
                if field_name == 'male_family_history_other':
            ...
            if family_history_text:
                self.male_family_history = '<br>'.join(family_history_text)
            else:
                self.male_family_history = False
        else:
            self.male_family_history = False
        ...
        if family_history:
            ...
            for field_name, (field_label, custom_field_name) in fields_to_process.items():
                field_records = getattr(family_history, field_name)
                custom_field = getattr(family_history, custom_field_name)
                if field_name == 'female_family_history_other':
                    family_members_list = field_records
                    if family_members_list:
                        field_text = f'<strong style="font-weight: 700;">{field_label}:</strong>({family_members_list})'
                        family_history_text.append(field_text)
            ...
            if family_history_text:
                self.female_family_history = '<br>'.join(family_history_text)
            else:
                self.female_family_history = False
        else:
            self.female_family_history = False
    ...
    #     if family_history:
    #         fields_to_process = {
    #             'male_diabetes_ids': 'Diabetes',
    #             'male_malignancies_ids': 'Malignancies',
    #             'male_hypertension_ids': 'Hypertension',
    #             'male_mental_illness_ids': 'Mental Illness',
    #             'male_tuberculosis_ids': 'Tuberculosis',
    #             'male_abnormalities_ids': 'Abnormalities'
    #         }
    #
    #         family_history_text = []
    #         for field_name, field_label in fields_to_process.items():
    #             field_records = getattr(family_history, field_name)
    #             if field_records:
    #                 family_members_list = [rec.name for rec in field_records]
    #                 if family_members_list:
    #                     field_text = f'<strong style="font-weight: 700;">{field_label}:</strong> ({", ".join(family_members_list)})'
    #                     family_history_text.append(field_text)
    #
    #         if family_history_text:
    #             self.male_family_history = '<br>'.join(family_history_text)
    ...
    #     if family_history:
    #         fields_to_process = {
    #             'female_diabetes_ids': 'Diabetes',
    #             'female_malignancies_ids': 'Malignancies',
    #             'female_hypertension_ids': 'Hypertension',
    #             'female_mental_illness_ids': 'Mental Illness',
    #             'female_tuberculosis_ids': 'Tuberculosis',
    #             'female_abnormalities_ids': 'Abnormalities'
    #         }
    #
    #         family_history_text = []
    #         for field_name, field_label in fields_to_process.items():
